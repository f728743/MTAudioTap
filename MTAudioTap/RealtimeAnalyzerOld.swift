...
class RealtimeAnalyzer {
    ...
    func analyse(
        ...
        // Ensure spectrumBuffer is initialized with the correct number of channels
        let channelCount = Int(ablPointer.count)
        var processingSamples = [Float](repeating: 0.0, count: fftSize)
        ...
        for i in 0 ..< channelCount {
            ...
            // Copy samples directly using memcpy
            _ = processingSamples.withUnsafeMutableBufferPointer { destBuffer in
                memcpy(destBuffer.baseAddress, data, fftSize * MemoryLayout<Float>.size)
            }
            
            let channelBuf = processingSamples.withUnsafeMutableBufferPointer { $0 }
            let channel = channelBuf.baseAddress!
            let amplitudes = fftChannel(channel)
            ...
            var spectrum = bands.map {
                findMaxAmplitude(
                    ...
                    with: Float(sampleRate) / Float(self.fftSize)
                ) * 5
            }
    ...
    // swiftlint: disable shorthand_operator
        private func fftChannel(_ channel: UnsafeMutablePointer<Float>) -> [Float] {
            // 2: Apply Hann window
